...
int main(int argc, char ** argv) {
	...
	//cudaStream_t stream[...];
	cudaStream_t streams[NR_CHUNKS];
	...
	// threadsPerBlock =... ;	//FIXME: uncomment and set - this variable is of type "unsigned int"
	//TODO: Define the number of blocks in the grid "numBlock" - assume one dimension	
	// numBlock = ...;		//FIXME: uncomment and set - this variable is of type "unsigned int"
	...
	size_t streamSize = (M*N)/NR_CHUNKS;
	for (i = 0 ; i < NR_CHUNKS ; i++) {
		
		size_t offset = i * streamSize;
		//TODO: Copy a chunk of "A"  (M / NR_CHUNKS rows) over a stream to the GPU in "dev_A"
		cudaMemcpyAsync(&dev_A[offset],&A[offset], streamSize*sizeof(float),
			cudaMemcpyHostToDevice, streams[i]);
		//TODO: Compute DGEMV on this chunk on the GPU over a stream
		cudaDGEMV_shmem <<< M/NR_CHUNKS, BLOCK_SIZE,0, streams[i] >>>(&dev_A[offset],dev_x,&dev_b[N/NR_CHUNKS*i], M/NR_CHUNKS,N);
		//TODO: Copy the produced chunk of "b" (M / NR_CHUNKS elements) over a stream from the GPU from "dev_b" 
		cudaMemcpyAsync(&b[M/NR_CHUNKS*i],&dev_b[M/NR_CHUNKS*i],M/NR_CHUNKS * sizeof(float), cudaMemcpyDeviceToHost, streams[i]);
		// printf("stream %d: M chunck size:%d, A offset:%ld \n",i, M/NR_CHUNKS*i,offset);
	}
